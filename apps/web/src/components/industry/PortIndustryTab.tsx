...
function Overview({data,nationalStats,onCompany,onRegions}:{data:PortOverviewResponse;nationalStats:NationalPortStatsResponse|null;onCompany:(symbol:string)=>void;onRegions:()=>void}){
  ...
  const regionOf=(name:string)=>/Quảng Ninh|Hải Phòng|Thái Bình|Nam Định/i.test(name)?"Bắc":/Thanh Hóa|Nghệ An|Hà Tĩnh|Quảng Bình|Quảng Trị|TT\. Huế|Huế/i.test(name)?"Bắc Trung Bộ":/Đà Nẵng|Quảng Nam|Quảng Ngãi|Quy Nhơn|Nha Trang/i.test(name)?"Trung":"Nam";
  const regionClass=(name:string)=>{const r=regionOf(name);return r==="Bắc"?"north":r==="Bắc Trung Bộ"?"north-central":r==="Trung"?"central":"south"};
  ...
  return <>
    ...
    <section className="portPanel portAuthorityPanel"><div className="portSectionHead portAuthorityHead"><div><span className="portSectionIndex">01</span><div><h3>Hoạt động theo Cảng vụ</h3><p>So sánh nhanh quy mô hoạt động theo số liệu VIMAWA công bố.</p></div></div><div className="portTrendControls"><span className="portSortLabel">Sắp xếp</span><div className="portMetricSwitch"><button className={sort==="cargoTons"?"active":""} onClick={()=>setSort("cargoTons")}>Hàng hóa</button><button className={sort==="containerTeu"?"active":""} onClick={()=>setSort("containerTeu")}>Container</button><button className={sort==="shipCalls"?"active":""} onClick={()=>setSort("shipCalls")}>Lượt tàu</button><button className={sort==="gtTotal"?"active":""} onClick={()=>setSort("gtTotal")}>GT</button></div></div></div>
      {!rows.length?<div className="portEmpty">Chưa đọc được bảng Cảng vụ từ nguồn official.</div>:<div className="portAuthorityTableWrap"><table className="portAuthorityTable"><thead><tr><th>Cảng vụ</th><th>Vùng</th><th className="num">Lượt tàu</th><th className="num">GT</th><th className="num">Hàng hóa</th><th className="num">Container</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.authority}><td><div className="portAuthorityName"><span className={`portRegionDot ${regionClass(r.authority)}`}/><div><strong>{r.authority}</strong><small>#{i+1} theo {sort==="cargoTons"?"hàng hóa":sort==="containerTeu"?"container":sort==="shipCalls"?"lượt tàu":"GT"}</small></div></div></td><td><span className={`portRegionPill ${regionClass(r.authority)}`}>{regionOf(r.authority)}</span></td><td className="num"><strong>{fmt(r.shipCalls,0)}</strong><small>lượt</small></td><td className="num"><strong>{fmtCompact(r.gtTotal)}</strong><small>GT</small></td><td className="num"><strong>{fmtCompact(r.cargoTons)}</strong><small>tấn</small></td><td className="num">{teuCell(r.containerTeu)}</td></tr>)}</tbody></table></div>}
      <div className="portAuthorityLegend"><span><i className="dot official"/>Nguồn official VIMAWA</span><span><b>0 TEU</b> = nguồn ghi nhận không phát sinh container; vẫn có thể có hàng rời/hàng lỏng/hàng tổng hợp.</span><span><b>—</b> = thiếu/không đọc được dữ liệu.</span></div>
      <p className="portFootnote">Nguồn: VIMAWA – “Tàu thuyền ra, vào cảng biển”. GT, hàng hóa và TEU là các chỉ tiêu khác nhau; có hàng hóa và lượt tàu không đồng nghĩa phải có container.</p>
    </section>
